    ...
    private static String buildSignSrc(JSONObject jsonObject) {
        ...
        for (String key : keys) {
            ...
            if (value_type!=null && value_type.getClass().getSimpleName().equals("JSONObject") ){
                values = JSONObject.toJSONString(value_type,SerializerFeature.MapSortField);
            }

            else if (value_type!=null && value_type.getClass().getSimpleName().equals("JSONArray") )
            {
                values = JSONArray.toJSONString(value_type, SerializerFeature.MapSortField);
            }
