...
public class FindSignTest {
    public static String signWithMD5(String jsonString,String secret){

        try {
            // 生成待签名串
//            JSONObject jsonObject = JSON.parseObject(JSON.toJSONString(jsonString, SerializerFeature.MapSortField));
            JSONObject jsonObjectTest = JSON.parseObject(jsonString);
           String test1=JSON.toJSONString(jsonObjectTest,SerializerFeature.MapSortField);
